...
class MockProvider {
    ...
    setupDefaultResponses() {
        this.responses.set('spec', `# Project Specification
...
- [ ] Criterion 2
- [ ] Criterion 3`);
        this.responses.set('plan', `# Implementation Plan
...
Review changes and ensure quality.`);
        this.responses.set('implement', `I've analyzed the plan and will implement the required changes. Here's the patch:
...
+  return newFeature();
 ...
 \`\`\``);
        this.responses.set('review', `# Code Review Summary
...
- Should we add more integration tests?`);
    ...
    async chat(options) {
        ...
        if (lastMessage.toLowerCase().includes('spec')) {
