...
import { readFile, writeFile } from '../../fs/file-ops.js';
import { applyPatch, createPatchInfo } from '../../fs/patch.js';
import { getGitDiff, stageFiles, isGitRepo } from '../../tools/git.js';
import type { CommandResult, PatchInfo } from '../../types/state.js';
...
export class ImplementCommand extends BaseCommand {
  ...
  async execute(): Promise<CommandResult> {
    ...
    try {
      ...
      const plan = await readFile(context.planPath);
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        const diff = result.data as { diff?: string };
        if (diff?.diff) {
      ...
      if (isRepo) {
        const gitDiff = await getGitDiff(this.cwd);
  ...
  private parsePlanSteps(plan: string): string[] {
    const steps: string[] = [];
  ...
  private async confirmApply(_patch: string): Promise<boolean> {
  ...
  private async applyDiffs(patchContent: string): Promise<CommandResult> {
    ...
    for (const [filePath, patchLines] of files) {
      ...
      const result = await applyPatch(fullPath, patchText, this.options.dryRun);
